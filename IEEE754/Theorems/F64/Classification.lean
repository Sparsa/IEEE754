/-
  IEEE754.Theorems.F64.Classification
  =====================================
...
theorem classify_exclusive (f : F64) :
...
inductive F64Class where | zero | subnormal | normal | inf | nan

def classify (f : F64) : F64Class :=
...
theorem finite_classify (f : F64) (hNaN : f.isNaN = false) (hInf: f.isInf = false) :
...
theorem expZero_ne_expMax (f : F64) :
    ¬(f.expIsZero = true ∧ f.expIsMax = true) := by
  simp [expIsZero, expIsMax]
  intro h; simp [h]

theorem isZero_false_of_isSubnormal (f : F64) (h : f.isSubnormal = true) :
    f.isZero = false := by
  simp [isZero, isSubnormal] at *
  obtain ⟨hexp, hmant⟩ := h
  simp [hexp, hmant]

theorem isZero_false_of_isNormal (f : F64) (h : f.isNormal = true) :
    f.isZero = false := by
  simp [isZero, isNormal, expIsZero, expIsMax] at *
  obtain ⟨hne0, _⟩ := h
  cases hm : f.mantIsZero <;> simp [hne0]

theorem isZero_false_of_isInf (f : F64) (h : f.isInf = true) :
    f.isZero = false := by
...
theorem isZero_false_of_isNaN (f : F64) (h : f.isNaN = true) :
    f.isZero = false := by
...
theorem isSubnormal_false_of_isNormal (f : F64) (h : f.isNormal = true) :
...
theorem isSubnormal_false_of_isInf (f : F64) (h : f.isInf = true) :
    f.isSubnormal = false := by
...
theorem isSubnormal_false_of_isNaN (f : F64) (h : f.isNaN = true) :
    f.isSubnormal = false := by
...
theorem isNormal_false_of_isInf (f : F64) (h : f.isInf = true) :
    f.isNormal = false := by
  simp [isNormal, isInf, expIsMax] at *
  intros exp_zero
  exact h.1

theorem isNormal_false_of_isNaN (f : F64) (h : f.isNaN = true) :
    f.isNormal = false := by
  simp [isNormal, isNaN, expIsMax] at *
  intros
  exact h.1

theorem isInf_false_of_isNaN (f : F64) (h : f.isNaN = true) :
    f.isInf = false := by
...
theorem isInf_false_of_isZero (f : F64) (h : f.isZero = true) :
  f.isInf = false := by
...
theorem isInf_false_of_isFinite (f:F64) (h:f.isFinite = true) : f.isInf = false := by
  simp [isInf, isFinite, expIsMax] at *
  intro h1
  rw [h1] at h
  contradiction

theorem isInf_false_of_isNormal (f : F64) (h : f.isNormal = true) :
    f.isInf = false := by
...
theorem isInf_false_of_isSubnormal (f:F64) (h : f.isSubnormal = true) :
  f.isInf = false := by
...
theorem isNormal_false_of_isSubnormal (f:F64) (h: f.isSubnormal = true) :
  f.isNormal = false := by
...
theorem isNaN_false_of_isInf (f:F64) (h:f.isInf = true) :
  f.isNaN = false := by
  simp [isNaN,isInf] at *
...
theorem isNaN_false_of_isZero (f:F64) (h:f.isZero = true) :
  f.isNaN = false := by
...
theorem isNaN_false_of_isSubnormal (f:F64) (h:f.isSubnormal = true) :
  f.isNaN = false := by
  simp [isNaN]
...
theorem isNaN_false_of_isNormal (f:F64) (h: f.isNormal = true):
  f.isNaN = false := by
  simp [isNaN]
  simp [isNormal] at h
  have ⟨hl,hr⟩ := h
...
theorem isNaN_false_of_isFinite (f:F64) (h: f.isFinite = true) :
  f.isNaN = false := by
  simp [isNaN, isFinite] at *
  intro h1
  simp_all

theorem isZero_false_of_isSubnormal' (f : F64) (h : f.isSubnormal) : ¬f.isZero := by
  simp [isZero_false_of_isSubnormal f h]

theorem biject_class_zero (f : F64) :
    f.isZero = true ↔ classify f = .zero := by
  constructor
  · intro h; unfold classify; rw [h]; simp
  · intro h
    ...
    | false =>
      ...
      | false =>
             ...
             | true => simp_all
             | false =>
                     rw [hSubN, hN] at h; simp at h
                     cases hInf : f.isInf with
                     | true => simp_all
                     | false => simp_all

theorem biject_class_nan (f:F64) :
  f.isNaN ↔ (classify f) = .nan := by
constructor
· intro h
  unfold classify
    ...
    | false =>
           ...
           | false =>
                   ...
                   | false =>
                     ...
                     rw [hNan,hZ,hsN,hN] at ce_f; simp at ce_f
                     rw [h] at ce_f; contradiction

theorem biject_class_inf (f:F64) :
  f.isInf ↔ (classify f) = .inf := by
constructor
· intro h
  unfold classify
  ...
  | false =>
    ...
    | false =>
           ...
           | false =>
                   ...
                   | true => simp_all
                   | false =>
                     rw [hN] at h; simp at h; simp_all

theorem biject_class_normal (f:F64) :
  f.isNormal ↔ (classify f) = .normal := by
constructor
·
  intro h
  unfold classify
  ...
  | false =>
    ...
    | false =>
            ...
            | false =>
                ...
                cases hInf : f.isInf with
                | true => simp_all
                | false => rw [hInf] at h; simp at h

theorem biject_class_subnormal (f:F64) :
  f.isSubnormal ↔ (classify f) = .subnormal := by
 constructor
 ·
  intro h
  unfold classify
